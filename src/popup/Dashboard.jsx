...
export default function Dashboard({ page, setpage }) {
  ...
  const [addingtofolderbackendcall, setaddingtofolderbackendcall] = useState(false)
  ...
  const handleCloseTab1 = (tabId) => {
    chrome.storage.local.get(["token69"], (result) => {
      if (result.token69) {
        ...
        setdeletinghistory(true);
        fetch(
          "https://extensionbackend.hamdidcarel.workers.dev/deletetab",
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
            ...
            <section className="mb-4">
                        ...
                        <div className="flex space-x-4 justify-items-center">
                          ...
                          {addingtofolder && (
                            ...
                            <input
                              ...
                              className={`form-checkbox h-3 w-3 text-blue-500 text-xs mr-3`}
                            />
                            </div>
                          )}
                          {/* {
                          ...
                          <span className="mb-1 text-xs">{tab.title}</span>
                        </div>
                      </li>
                    );
                  })}
              ...
              <button
                ...
                className={`w-full mt-0.5 py-2 ${
                  saveFolder || addingtofolderbackendcall ? "bg-blue-200" : "bg-blue-500"
                } ${
                  saveFolder || addingtofolderbackendcall ? "hover:bg-blue-200" : "hover:bg-blue-700"
                }  text-white rounded-md  focus:outline-none`}
              >
                {!saveFolder && !addingtofolder && "Save Current Tabs as Record"}
                {saveFolder&& !addingtofolder && "Saving..."}
                {addingtofolder && !addingtofolderbackendcall &&`Add selected tabs to ${currentfoldername}`}
                {addingtofolder&& addingtofolderbackendcall && `Adding to ${currentfoldername}..`}
              </button>
            </section>
            <section className="mb-4">
              <div className="flex">
                ...
                <ul className="saved-windows-list overflow-y-auto h-48 border rounded-md px-2 py-1">
                  ...
                  {!loadfolders &&
                    folders.map((tab) => {
                      ...
                      return (
                        <li>
                          {tryfoldel=== tab.id &&<li className="flex flex-col justify-center hover:bg-gray-300 p-2 rounded-md shadow-2xl cursor-pointer"
                          key={tab.id}>
                              ...
                              if (result.token69) {
                                ...
                                fetch(
                                  "https://extensionbackend.hamdidcarel.workers.dev/getmantabs",
                                  {
                                    ...
                                    body: JSON.stringify({ folder_id: tab.id }),
                                  }
                                )
                                  .then((response) => response.json())
                                  .then((data) => {
                  ...
                  {!loadfoldertabs && foldertabs.length > 0 && (
                    <li className="flex">
                      ...
                      <div 
                        ...
                        className={`flex-1 flex flex-col justify-center hover:bg-gray-300 p-2 rounded-md shadow-2xl text-blue-600 ${addingtofolder?"":"cursor-pointer"} `}
                      >
                        {!addingtofolder &&
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-items-center">
                            ...
                            >
                              {!deletinghistory&& history[0].tabid!=0 &&<svg
                                className="mr-1"
                                xmlns="http://www.w3.org/2000/svg"
                                width="13"
                                height="15"
                              ...
                              fetch(
                                ...
                                .then((data) => {
                                  if (data.success == false) {
                                    ...
                                    chrome.runtime.sendMessage({ action: 'deleteHistory', laststore: "laststore"});
                                  }
                                  setdeletehistory(0);
                                  setdeletinghistory(false);
                                })
